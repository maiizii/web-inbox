...
export default function BlockEditorAuto({
  ...
  function renderPlainWithImages(raw) {
    ...
    while ((m = re.exec(raw)) !== null) {
      ...
      out += `<img class="preview-img" src="${escapeHtml(
        m[2]
  ...
  function updatePreview(txt) {
    setPreviewHtml(renderPlainWithImages(txt));
  }
  ...
  function maybeRestoreFocus() {
    if (userManuallyBlurredRef.current || !shouldRestoreFocusRef.current) return;
    const ta = textareaRef.current;
  ...
  function syncLineNumbersPadding() {
    ...
    if (!ta || !inner) return;
    inner.style.top =
      (parseFloat(getComputedStyle(ta).paddingTop) || 0) + "px";
  }
  useEffect(() => {
    syncLineNumbersPadding();
    const onResize = () => {
  ...
  function detectOverflow() {
    ...
    overflowCheckTimerRef.current = requestAnimationFrame(() => {
      ...
      if (pv) {
        pv.classList.toggle(
          "no-v-scroll",
          pv.scrollHeight <= pv.clientHeight + 1
        );
      }
    });
  }
  useEffect(() => {
    detectOverflow();
  }, [content, showPreview, previewMode, splitRatio]);
  ...
  return (
    ...
    >
      {/* 顶部标题条：强制采用表面色背景与主题文本色，标题文字永远可读 */}
      <div
      ...
      >
        <div
          className="flex-1 text-lg font-semibold truncate select-none"
