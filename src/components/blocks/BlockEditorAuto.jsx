...
export default function BlockEditorAuto({
  ...
  function startDividerDrag(e) {
    if (!showPreview || mobileMode) return;
    e.preventDefault();
  ...
  }, [showPreview, syncScrollEnabled, previewMode, content, mobileMode]);
  ...
  const TopBar = (
    ...
    >
      {mobileMode && (
        <>
          <button className="btn-outline-modern !px-3 !py-1.5" onClick={() => onMobileBack && onMobileBack()}>返回列表</button>
          <button className="btn-outline-modern !px-3 !py-1.5" onClick={() => onMobilePreviewRequest && onMobilePreviewRequest()}>预览</button>
        </>
      )}
      <div className="flex-1 text-lg font-semibold truncate select-none" style={{ color: "var(--color-text)" }}>
        {derivedTitle}
      </div>
