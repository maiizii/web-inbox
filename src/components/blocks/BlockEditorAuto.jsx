 ...
 *  - value: 编辑器文本（可选，受控）
 *  - onChange: 编辑器文本变更回调
 *  - renderPreview: (value) => ReactNode | html string (预览渲染)
 */
export default function BlockEditorAuto({ value = "", onChange, renderPreview }) {
  const containerRef = useRef(null);
  const draggingRef = useRef(false);
  const [leftPct, setLeftPct] = useState(50); // 左侧占比（%）
  ...
  useEffect(() => {
    function onPointerMove(e) {
      ...
      if (!rect) return;
      // e.clientX works for pointer events; for safety get touches if needed (pointer covers both)
  ...
  function startDrag(e) {
    // use Pointer Events: onPointerDown -> begin dragging
    e.preventDefault();
    draggingRef.current = true;
    document.body.style.cursor = "col-resize";
    document.body.style.userSelect = "none";
    // try to capture pointer to ensure continuous events on some browsers
    try {
  ...
  return (
    <div ref={containerRef} className="editor-container" style={{ display: "flex", height: "100%" }}>
      <div
        className="editor-pane"
        style={{
          width: `${leftPct}%`,
      ...
      >
        {/* 你原来的编辑器可以替换这里（例如 CodeMirror/Markdown 编辑器） */}
        <textarea
          className="editor-textarea"
          value={value}
          onChange={handleChange}
          style={{ width: "100%", height: "100%", boxSizing: "border-box", padding: 12, border: "none", resize: "none" }}
        />
      </div>
      ...
      <div className="preview-pane" style={{ flex: 1, overflow: "auto", boxSizing: "border-box", padding: 12 }}>
        {/* renderPreview 可以是函数或已经生成的 HTML */}
