...
export default function ChangePasswordModal({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4"
         ...
         onClick={onClose}>
      <div className="w-full max-w-md rounded-xl shadow-lg border"
           style={{ background: "var(--color-surface)", color: "var(--color-text)", borderColor: "var(--color-border)" }}
           onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--color-border)" }}>
          <div className="text-base font-semibold">修改密码</div>
          <button className="btn-ghost-modern !px-2 !py-1" onClick={onClose}><X size={18} /></button>
        </div>

        <div className="px-4 py-4 space-y-3">
          <div>
            <div className="text-xs mb-1 text-slate-500 dark:text-slate-300">当前密码</div>
            <input type="password" className="input-modern" value={oldPwd}
                   onChange={e => setOldPwd(e.target.value)} placeholder="输入当前密码" autoFocus />
          </div>
          <div>
            <div className="text-xs mb-1 text-slate-500 dark:text-slate-300">新密码</div>
            <input type="password" className="input-modern" value={newPwd}
                   onChange={e => setNewPwd(e.target.value)} placeholder="至少 8 位，建议包含字母与数字" />
          </div>
          <div>
            <div className="text-xs mb-1 text-slate-500 dark:text-slate-300">确认新密码</div>
            <input type="password" className="input-modern" value={confirmPwd}
        ...
        <div className="px-4 py-3 flex items-center justify-end gap-2 border-t" style={{ borderColor: "var(--color-border)" }}>
          <button className="btn-outline-modern" onClick={onClose}>取消</button>
          <button className="btn-primary-modern min-w-[96px] justify-center" onClick={submit} disabled={loading}>
