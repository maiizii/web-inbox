...
export async function onRequestPost({ request, env }) {
  try {
    ...
    // 从 Cloudflare Access 取登录身份（你现在页头显示的邮箱）
    const email =
    ...
    if (res.success === false) throw new Error("数据库更新失败");
    return json({ ok: true });
  } catch (e) {
    const msg = e?.message || "内部错误";
    // 统一 500，前端会映射
...
async function pbkdf2Hash(password, iter = 100_000) {
  ...
  // 存储格式：pbkdf2$<iter>$<saltB64>$<hashB64>
...
async function verifyPassword(input, storedHash, saltB64) {
  ...
  if (storedHash.startsWith("pbkdf2$")) {
    ...
    return subtleEqual(calc, parts[3]);
  }

  // 2) 兼容：存了 salt 列（旧实现可能 sha256(password+salt)）
  if (saltB64) {
